...
from vnpy.trader.utility import get_folder_path,load_json, save_json
recording_list = CheckFutureNews().recording_list
recording_list = [X for X in recording_list if "HUOBIF" in X]
#交易host
TRADING_HOST = ["1token.trade/api","api.1token.trade","cdn.1tokentrade.cn/api"][2]
REST_HOST = f"https://{TRADING_HOST}"
DATA_WEBSOCKET_HOST = f"wss://{TRADING_HOST}/v1/ws/tick-v3"    #tick，tick-v3
...
EXCHANGE_CHOOSE = 7 #交易所选择,huobif
#-------------------------------------------------------------------------------------------------
class Onetoken_2_Gateway(BaseGateway):
    """
    ...
    def __init__(self, event_engine):
        """Constructor"""
        super(Onetoken_2_Gateway, self).__init__(event_engine, "1TOKEN2")
    ...
    def connect(self):
        ...
        exchange = list(EXCHANGE_VT2ONETOKEN.values())[EXCHANGE_CHOOSE]   #huobif
...
class OnetokenRestApi(RestClient):
    ...
    def send_order(self, req: OrderRequest):
        """"""
        self.one_token_2_order_id = self.exchange + "/" + req.symbol + "-" + str(self.connect_time + self._new_order_id())
        self.order_id_data[req.vt_symbol] = self.one_token_2_order_id
        #保存委托单号到本地
        save_json("one_token_2_order_id.json",self.order_id_data)
        #保存委托类到本地
        ...
        data = {
            ...
            "client_oid": self.one_token_2_order_id
        }

        if req.offset == Offset.CLOSE:
            data["options"] = {"close": True}
        data = json.dumps(data)
        order = req.create_order_data(self.one_token_2_order_id, self.gateway_name)
    ...
    def on_send_order(self, data, request):
        """收到发单推送"""
        self.sen_order_data = load_json("one_token_2_order_map.json")
        ...
        save_json("one_token_2_order_map.json",self.sen_order_data)
    #-------------------------------------------------------------------------------------------------   
...
class OnetokenDataWebsocketApi(WebsocketClient):
    ...
    def __init__(self, gateway):
        ...
        save_json("onetoken_2_data_status.json",True)     #设置onetoken_2_data_status默认为True
    ...
    def on_login(self, data: dict):
        ...
        if self.data_status_count >= 2:
            save_json("onetoken_2_data_status.json",False)
    #-------------------------------------------------------------------------------------------------   
...
class OnetokenTradeWebsocketApi(WebsocketClient):
    ...
    def on_info(self, data: dict):
        ...
        for position_data in position_detail:
            ...
            if account_type == "future-spot":
                #账户参数
                current_date,current_time = datetime.now().date(),datetime.now().time()
                if position_data["total_amount"]:
                    account = AccountData(
                        accountid = position_data["contract"] + "_" + self.exchange.upper() + "_" + self.account,
                        commission = self.commission,
                        date = str(current_date),
                        time = str(current_time),
                        pre_balance=round(data["balance"],3),                      #人民币计价总资金
                        balance = round(position_data["total_amount"],3),           #本币计价总资金
                        available=round(position_data["available"],3),                     #本币计价可用资金
                        frozen=position_data['frozen_position'],
                        close_profit =position_data["realized_profit"],
                        position_profit =position_data["unrealized_profit"],
                        gateway_name=self.gateway_name) 
                    ...
                    #保存账户资金信息
                    one_token_account_path = CheckFutureNews().ctp_account_path.replace("ctp_account","one_token_1_account")
    ...
    def on_order(self, data: dict):
        ...
        for order_data in data:
            self.commission = order_data["commission"]      #手续费              
            contract_symbol = order_data["contract"]        #合约代码
            exchange_str, symbol = contract_symbol.split("/")
            order_date,order_time =order_data["entrust_time"].split('T')
            if order_data["client_oid"]:
                orderid = order_data["client_oid"]
                #如果收到交易所client_oid清空本地order_id数据
                #save_json("one_token_2_order_id.json",{})
            else:
                try:
                    orderid = load_json("one_token_2_order_id.json")[f"{symbol}_{exchange_str.upper()}"]      #委托单号
                except:
                   return
            order = OrderData(
                symbol=symbol,
