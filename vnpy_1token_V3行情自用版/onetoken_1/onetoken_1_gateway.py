...
recording_list = [X for X in recording_list if "OKEF" in X]
#交易host
...
EXCHANGE_CHOOSE = 4 #交易所选择,okef
#-------------------------------------------------------------------------------------------------
class Onetoken_1_Gateway(BaseGateway):
    """
    ...
    def __init__(self, event_engine):
        """Constructor"""
        super(Onetoken_1_Gateway, self).__init__(event_engine, "1TOKEN1")
    ...
    def connect(self):
        ...
        exchange = list(EXCHANGE_VT2ONETOKEN.values())[EXCHANGE_CHOOSE]   #okef
...
class OnetokenRestApi(RestClient):
    ...
    def send_order(self, req: OrderRequest):
        """"""
        self.one_token_1_order_id = self.exchange + "/" + req.symbol + "-" + str(self.connect_time + self._new_order_id())
        #保存委托类到本地
        ...
        data = {
            ...
            "client_oid": self.one_token_1_order_id
        }

        if req.offset == Offset.CLOSE:
            data["options"] = {"close": True}
        data = json.dumps(data)
        order = req.create_order_data(self.one_token_1_order_id, self.gateway_name)
    ...
    def on_send_order(self, data, request):
        """收到发单推送"""
        self.sen_order_data = load_json("one_token_1_order_map.json")
        ...
        save_json("one_token_1_order_map.json",self.sen_order_data)
    #-------------------------------------------------------------------------------------------------   
...
class OnetokenDataWebsocketApi(WebsocketClient):
    ...
    def __init__(self, gateway):
        ...
        save_json("onetoken_1_data_status.json",True)     #设置onetoken_1_data_status默认为True
    ...
    def on_login(self, data: dict):
        ...
        if self.data_status_count >= 2:
            save_json("onetoken_1_data_status.json",False)
    #-------------------------------------------------------------------------------------------------   
